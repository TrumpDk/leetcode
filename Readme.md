...
#### 基数排序 [2343]
#### 寻找满足条件的子列和是不是更加该用前缀和[1124]
#### 麻痹，居然在锯齿遍历栽了[103] BFS
#### [207] 拓补排序 DFS 拓补排序没了解过 不知道怎么搞
...
#### 如何实现循环数组 [503]
#### 去重遇到顺序问题 使用数组或者字典 存储出现次数和位置 从而达到辅助目的 以空间换取时间  [316]
#### [1130] [1673]  子集问题 是弱项 得重点加固下
#### [2104] [84] 单调栈解决子集问题 如何将他们联系起来 现阶段没有任何思路 哎。。。草
#### [1996] 排序以及单调栈的问题 思路就是不断拆解纠结成一团的问题 将干扰条件拆分成为子条件 (先将数组排序 然后通过单调栈解决问题)
#### [2289] 单调栈和线段树 线段树新天地
#### [907] 前驱后继数组 没刷过啊
#### [1504] [2289] [面试题33] 单调栈老大难题  二叉树遍历思维 以前刷到过一个后序倒序遍历的题   有很多题时思维非常重要的
#### [1177] 状态压缩 什么是状态压缩 [1292] 二位单调栈 二位滑动窗口 [1310] 异或运算 [1314] 二维单调栈或者二维滑动窗口 [1371] 前缀和加状态压缩 都特么没刷过
